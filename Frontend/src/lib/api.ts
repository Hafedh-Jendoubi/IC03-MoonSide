...
export const auditApi = {
  ...
  // Organization Service audit logs
  getOrgLogs: (params?: {
    page?: number
    size?: number
    userId?: string
    ...
    if (params?.action) query.set('action', params.action)
    if (params?.success != null) query.set('success', String(params.success))
    return apiFetch<PageResponse<AuditLogResponse>>(`/organizations/audit-logs?${query.toString()}`)
  },

  getOrgStats: () => apiFetch<AuditLogStats>('/organizations/audit-logs/stats'),
}
